import prisma from "../db/database.js";

class Movies {
    static async getAll(req, res){
        try{
            const data = await prisma.movies.findMany();
            res.status(200).json({
                message : "All Movies Successfully being presented",
                data
            });
        } 
        catch (error) {
            console.log(error);
            res.status(500).json ({
                message : "Internal sever error"
            });
        }
    }
    static async getpagi(req, res){
        try{
            const {page} = req.query;
            const limit = 10;
            const skip = (page-1)*limit;

            const data = await prisma.movies.findMany({
                skip: skip,
                take: limit
            });
            res.status(200).json({
                message : `Movies Successfuly Presented on page ${page}`,
                data: data
            });
        } 
        catch (error) {
            console.log(error);
            res.status(500).json ({
                message : "Internal sever error"
            });
        }
    }
    static async register(req, res) {
        try {
            const { title, genres, year } = req.body;

            if (!title || !genres || !year) return res.status(400).json({
                message: "Invalid input"
            });

            const existingMovies = await prisma.movies.findFirst({
                where: {
                    title: title
                }
            });
            if (existingMovies) return res.status(400).json({
                message: "Movie already exists"
            });
            const registerMovie = await prisma.movies.create({
                data: {
                    title: title,
                    genres: genres,
                    year: year
                }
            });
            res.status(200).json({
                message: "Movie successfuly registered",
                data: registerMovie
            });
        } catch (error) {
            console.log(error);
            res.status(500).json({
                message: "Internal server error"
            });
        }
    }
    static async put (req,res){
        try {
            const {id} = req.params;
            const {title, genres, year} = req.body
            if ( !id ) return res.status(404).json ({
                message: "Data Movie Not Found"
            })
            if (!title || !genres || !year) return res.status(400).json({
                message: "Invalid input"
            });
            const editMovie = await prisma.movies.update({
            where: {
                id: parseInt(id)
            }, data: {
                title: title,
                genres: genres,
                year: year
            }
            });
            res.status(200).json({
                message: "Data Movie Successfully Updated",
                data: editMovie
            }); 
        } catch (error) {
            console.log(error);
            res.status(500).json({
                message: "Internal Server Error"
            });
        }
    }
    static async delete (req,res){
        try {
            const {id} = req.params;
            if(!id) return res.satus(404).json({
                message: "Data Movie Not Found"
            });
            const deleteMovie=await prisma.movies.delete({
                where: {
                    id: parseInt(id)
                }
            });
            res.status(200).json({
                deleteMovie
            });
        } catch (error) {
            console.log(error);
            res.status(500).json({
                message: "Internal Sever Error"
            });
        }
    }
}
export default Movies;